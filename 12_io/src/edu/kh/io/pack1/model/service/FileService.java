...
public class FileService {
	...
	public void method1() {
		...
		// 경로 제일 앞에 "/" 의 의미 : 최상위 폴더(== root, 절대 경로의 기준점)  
		...
		if(!directory.exists()) { // 폴더가 존재하지 않으면
			...
			System.out.println(directory.getName()); // 마지막 폴더 이름 출력 (20240926)
			System.out.println(directory.getPath()); // 경로 출력 (\io_test\20240926)
	...
	public void method2() {
		...
		if(!file.exists()) { // 폴더가 존재하지 않으면
			...
			try {
				...
				// -> 예외처리 필수
				if(file.createNewFile()){ // 파일을 생성했다, 잘 됐다면 true
	...
	public void method3() {
		...
		for(File f: files) {
			...
			//System.out.println(lastModified); // 1727317029349
	 ...
	 * 파일 시스템과의 일반적인 상호작용이기 때문에 스트림 사용할 필요 없음
	 * 
	 * -> 단순 폴더 생성, 삭제는 스트림이 필요하지 않다 ~~
	 * 
	 * */
}
