...
public class Company implements ManagementSystem{
	...
	public void addPerson(Person person) { // 업캐스팅
		...
		if(person instanceof Employee && employeeCount < employees.length) {
			employees[employeeCount++] = (Employee)person; // Employee로 다운캐스팅 
			System.out.println("직원이 추가되었습니다 - " + person.getInfo()); // 동적바인딩
		} else {
			System.out.println("인원이 모두 충원되어 더 이상 추가 못 함");
		}
		...
		for(int i = 0; i < employeeCount; i++) { // 현재 등록된 직원 수 만큼만 반복
			...
			if(employees[i].getId().equals(id)) {
				...
				for(int j = i; j < employeeCount -1; j++) {
					// 삭제한 요소가 있는 i번째 인덱스부터
					// 배열에 존재하는 직원 마지막 요소까지 순차 접근
					employees[j] = employees[j + 1];
					// 배열 내 모든 후속 요소를 왼쪽으로 한 칸씩 이동함
				}
				
				employees[--employeeCount] = null;
				// employeeCount 변수를 감소시켜 배열의 마지막 요소를 null로 설정하여 직원 수 줄이기
				return;
			}
		}
