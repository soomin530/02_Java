...
public class ToyFactory {
	...
	public ToyFactory() { // 기본생성자
		...
		// 1. 일반 배열 사용
		// toySet.add(new Toy("마미롱레그", 8, 36000, "분홍색", "19950805", addMaterials(new int[] materials)));
	...
	/** 매개변수로 전달받은 값들을
	 * 재료 저장 Map에 있는지 확인하고
	 * 있다면 Set 형태로 반환하는 메서드
	 * 
	 * 가변인자 작성법 : 자료형...변수명
	 * -> 매개변수 수가 정확히 몇 개인지 특정할 수 없을 때 사용
	 * @return
	 */
	public Set<String> addMaterials(Integer...materials/* List<Integer> materials */  /* int[] meterials */) {
...
public void displayMenu() {
		...
		} while(menuNum != 0); // menuNum이 0이 아닐 때 반복
	...
	public void createNewToy() throws Exception{
		...
		for(Toy existingToy : toySet) {
			if(existingToy.getName().equals(name)) { // String 형이기 때문에 equals 사용
		...
		while(true) {
			System.out.print("추가할 재료의 번호를 입력하세요 (종료하려면 q를 입력하세요) : ");
			
			try {
				String input = sc.next(); // 종료 버튼이 q 이기 떄문에 일단 String으로
				
				if(input.equals("q")) { // 종료 시
					break;
				}
				
				// String 형인 input을 int형 정수값으로 변환해줌 (Integer Wrapper 클래스 이용)
				int materialKey = Integer.parseInt(input); // "1" -> 1
				String material = materialMap.get(materialKey); // "면직물"
				// 입력받은 재료 고유번호로 map에 저장된 재료명(value)을 얻어옴
				
				if(material != null) { // 재료명이 null이 아니라면
					
					// materials(재료 저장할 Set 객체) Set 객체에 해당 재료명이 존재하는지
					if(materials.contains(material)) {
						System.out.println("이미 추가된 재료입니다. 다른 재료를 선택하세요.");
						
					} else { // Set 에 중복된 재료가 없으면 새로 추가
						materials.add(material);
						System.out.println("재료가 추가되었습니다 : " + material);  
					}
					
				} else { // 재료명 null == 없는 재료고유번호를 입력했다는 의미
	...
	public void deleteToy() throws Exception {
		...
		for(Toy toy : toySet) {
			if(toy.getName().equals(toyName)) {
				toySet.remove(toy); // toy 를 삭제해야 toySet에서 삭제됨
				// Set.remove(Object);
				flag = true;
				break;
				// 1. 효율 향상을 위해 break;
				// 2. 순회 중인 컬렉션 객체가 수정되어 충돌한다는 예외가 발생
	...
	public void addMaterial() throws Exception{
		...
		if(materialMap.containsKey(key)) { // materialMap에 이미 존재하는 Key 인지 확인
			...
			if(input.equalsIgnoreCase("Y")) {  
				...
				materialMap.put(key, material); // 같은 key로 추가 시 value를 덮어쓰기함
	...
	public void displayToysByManufactureDate() {
		...
		toyList.sort(Comparator.comparing(Toy::getManufactureDate)); // 제조일 기준으로 오름차순 정렬됨
	...
	public void displayToysByAge() {
		...
		for(Toy toy : toySet) {
			...
			// 				   해당 key가 존재하지 않는 경우에만 전달받은 값을 추가함
			// -> Map에 해당 연령의 리스트가 없는 경우에만 새로운 List 생성하여 추가
			
			toysByAge.get(age).add(toy);
			// toysByAge.get(age) -> age(K)에 맞는 Value 반환 -> List
			// List.add(toy) -> 전달된 toy 객체를 List 에 추가
		}
		...
		for(Entry<Integer, List<Toy>> entry : toysByAge.entrySet()) {
			...
			List<Toy> toyList = entry.getValue(); // 해당 entry의 value(List)를 얻어옴
