...
public class MemberService {
	...
	public String signUp() {
		...
		int index = emptyIndex(); // memberArr배열에서 비어있는 인덱스 찾아서 return
		 						  // 없다면 -1 반환
	...
	public String selectMember() {
		...
		// 현재 로그인 한 회원의 정보를 출력할 문자열 만들어 반환
	...
	public int updateMember() {
		...
		// 3) 비밀번호를 입력받아서
		// 	  로그인 한 회원의 비밀번호와 일치하는지 확인
			System.out.print("비밀번호 입력 : ");
			String inputPw = sc.next();
			
			if(inputPw.equals(loginMember.getMemberPw())) { // 4) 비밀번호가 같을 경우
				
				// 로그인 한 회원의 이름, 나이, 지역 정보를 입력받은 값으로 변경 후
				// 1 반환
