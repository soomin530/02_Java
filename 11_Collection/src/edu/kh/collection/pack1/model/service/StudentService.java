...
public class StudentService {
	...
	// private List<Object> testList = new ArrayList<Object>();  // -> ctrl + space -> 맨 위 클릭해서 import
	...
	// 부모 타입(List) 참조변수(testList) = 자식 객체(ArrayList) 주소 대입
	// 다형성 중 업캐스팅 적용 (testList에 Object 타입을 받아서 ArrayList에 저장)
	 
	// <E> - 제네릭(Generics)
	// -> 컬렉션에 저장되는 객체 타입을 한 가지로 제한 : <E> == Element 
	...
	public void ex() {
		...
		// get은 반환형이 Object임 == 모든 객체를 반환할 수 있다 (다형성 적용해서)
		// Object List.get(index) : 리스트에서 index 번째 인덱스에 있는 객체를 반환
		
		System.out.println( testList.get(0) );
		// Student 객체의 주소 -> toString 오버라이딩 돼서
		// Student [name=홍길동, age=25, region=서울시 중구, gender=M, score=90] 출력
	...
	public String addStudent() throws InputMismatchException{
		...
		if(studentList.add(new Student(name, age, region, gender, score))) { 
			...
			// (반환형)                   -> 제네릭<Student>때문에
			//								 List에 추가할 수 있는 객체 타입이 Student로 제한됨
	...
	public void selectAll() {
		...
		*/
		...
		// 작성법
		// for(컬렉션 또는 배열에서 꺼낸 한 개의 요소를 저장할 변수 : 컬렉션명 또는 배열명 (반복 접근할 대상)) {}
		int index = 0;
	...
	public String updateStudent() throws InputMismatchException{
		System.out.println("================= 학생 전체 수정 ===================");
		
		System.out.print("인덱스 번호 입력 : ");
		int index = sc.nextInt();
		
		// 1) 학생 정보가 studentList에 있는지 검사 : 한 명도 없는지 검사
...
System.out.println("================= 학생 검색 (이름 부분 포함) ===================");
	...
	private void sortByName() {
		...
		// Comparing() 은 기본적으로 오름차순, 내림차순 원하면 reversed() 사용!
		// Comparator 인터페이스의 static 메서드인
		// comparing()을 사용하여 Comparator 객체 생성
