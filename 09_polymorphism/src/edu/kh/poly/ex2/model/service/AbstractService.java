...
public class AbstractService {
	...
	// Animal 부모 = Fish 자식(다형성 중 업캐스팅)
		...
		for(int i = 0; i < arr.length; i++) {
			arr[i].eat();
			// void edu.kh.poly.ex2.model.dto.Animal.eat () - 정적바인딩
			arr[i].breath();
		...
		// 프로그램 실행 시
		// 참조하고 있는 자식 객체의 오버라이딩 된
		// eat(), breath(), toString() 메서드 수행
		// -> 동적 바인딩 : 부모타입 참조변수로 메서드를 호출했지만
		// 				    실행해보면 자식 타입의 오버라이딩 된 메소드가 수행된다
