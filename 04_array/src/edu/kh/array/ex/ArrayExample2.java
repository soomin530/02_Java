...
public class ArrayExample2 {
	...
	public void shallowCopy() {
		...
		// 같은 배열 참조, 복사한 배열 값 변경 시 원본 배열 값도 변경
	...
	public void deepCopy() {
		...
		System.out.println(arr); // [I@59f95c5d
		System.out.println(copyArr1); // [I@5c8da962 
		// 서로 다른 배열을 참조하고 있음 
		
		System.out.println(Arrays.toString(arr)); // [1, 2, 3, 4, 5]
		System.out.println(Arrays.toString(copyArr1)); // [1, 2, 3, 4, 5]
		// 하지만 내용은 같음 !!
		
		// < 2. System.arraycopy >
		// (원본 배열, 원본 복사 시작 인덱스, 복사 배열,
		//  복사 배열의 삽입 시작 인덱스, 복사 길이);
		
		int[] copyArr2 = new int[arr.length]; // 5칸짜리 배열 생성
		System.arraycopy(arr, 0, copyArr2, 0, arr.length); // [1, 2, 3, 4, 5]
		
		System.out.println(Arrays.toString(copyArr2));
	
		// < 3. 복사할 배열 참조변수 = Arrays.copyOf(원본 배열, 복사할 길이); >
		int[] copyArr3 = Arrays.copyOf(arr, arr.length);
		System.out.println(Arrays.toString(copyArr3)); // [1, 2, 3, 4, 5]
		...
		// 복사된 배열 값 변경 시, 원본 배열은 변경 X 복사된 배열 값들만 변경됨
	}
