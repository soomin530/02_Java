...
public class VariableExample2 {
	...
	// 자료형 변수명  = 리터럴;
		...
		// 프로그래밍에서 대입되는 이 데이터(값 자체)를 리터럴이라는 단어로도 표현
		...
		// 뒤에 L을 붙여줌으로써 long 자료형인 것을 나타냄(소문자l, 대문자L 다 됨)
		
		
		float floatNumber = 1.2345F; // f 또는 F 뒤에 붙여줘야 됨
		// 1.2345 까지만 쓰면 double 자료형으로 인식함
		
		double doubleNumber = 3.141592;
		// double 이 실수형 중에서 기본형
		// 리터럴 표기법이 없는 실수는 double 로 인식
		// d를 뒤에 쓸 수는 있지만 기본형이기 때문에 안 적어도 됨
		...
		int $intNumber; // 문제는 없지만 개발자가 직접 이렇게 작성하지 않음
		int int_Number; // 자바는 카멜표기법 사용
		 				// _ 작성 표기법은 DB에서 사용
						//  ( DB에서는 MEMBER_NAME 이렇게 사용 )
